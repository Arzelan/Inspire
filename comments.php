		...
		<div class="inner">
			...
			<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
				...
				<div class="author-info <?php if ($comment_author) echo 'edit-off'; else echo 'edit-on'; ?>">
					<input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="20" required="required" tabindex="1" <?php if($req) echo "aria-required='true'"; ?> placeholder="*Name" />
					<input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="20" required="required" tabindex="2" <?php if($req) echo "aria-required='true'"; ?> placeholder="*Email" />
					<input type="text" name="url" id="url" value="<?php echo $comment_author_url; ?>" size="20" tabindex="3" placeholder="Site" />
				</div><!-- .author-info -->
				<?php endif; ?>
				<?php $shrink = ( $comment_author || $user_ID ) ? 'shrink' : ''; ?>
				<div class="comment-textarea error">
					<textarea id="comment" class="<?php echo $shrink; ?>" name="comment" cols="45" rows="1" maxlength="65525" aria-required="true" required="required" tabindex="4" placeholder="Express your thoughts ..."></textarea>
