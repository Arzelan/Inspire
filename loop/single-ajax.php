	...
	<div id="primary" class="content">
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemscope itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
			<header class="entry-header">
				...
				<div class="meta">Published on <time itemprop="datePublished" datetime="<?php echo get_the_date('c');?>"><?php echo get_post_time('Ag:i Y-m-d'); ?></time> in <a href="<?php echo esc_url( get_category_link( $cats[0]->term_id ) );?>"><?php echo $cats[0]->name; ?></a> with <?php comments_popup_link('0 comment', '1 comment', '% comment'); ?></div>
			</header>
